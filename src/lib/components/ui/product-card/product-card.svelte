<script lang="ts">
	...
	type ProductCardProps = WithElementRef<HTMLAttributes<HTMLDivElement>> & {
		imageUrl: string;
		imageAlt?: string;
		name: string;
		rating?: number;
	...
	<div class="flex flex-col gap-3 p-4">
		...
		{#if rating > 0}
			<div class="flex items-center gap-2">
				<span class="text-base font-medium text-foreground">{rating.toFixed(1)}</span>
		...
		<div class="flex items-center justify-between">
			<div class="flex items-baseline gap-0.5">
				...
				<span class="text-base font-normal text-foreground">{currency}</span>
			</div>
			<Button
				variant="outline"
				size="icon-lg"
